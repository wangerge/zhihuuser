...
class ZhihuSpider(Spider):
    ...
    uesr_url = 'https://www.zhihu.com/api/v4/members/{user}?include={include}'
    ...
    def start_requests(self):
        ...
        yield Request(self.uesr_url.format(user=self.start_user,include=self.user_query),callback=self.parse_user)
    ...
    def parse_follows(self, response):
        ...
        if 'data' in results.keys():
            for result in results.get('data'):
                yield Request(self.uesr_url.format(user=result.get('url_token'),include=self.user_query),callback=self.parse_user)
